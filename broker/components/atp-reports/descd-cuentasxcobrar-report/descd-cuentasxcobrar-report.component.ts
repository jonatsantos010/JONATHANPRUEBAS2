...
export class DescdCuentasxcobrarReportComponent implements OnInit {
    ...
    exportToExcel() {
        if (this.foundResults != null && this.foundResults.length > 0)
            this.excelService.generateReportCuentasxcobrarExcel(this.foundResults, 'ReporteCuentasxcobrar');
        else
        ...
        if (this.isError == true) {
            ...
            if ((this.bsValueIni != null && this.bsValueFin != null)) {
                if (new Date(this.bsValueIni) > new Date(this.bsValueFin)) {
                    this.isError = true;
                    this.UnselectedItemMessage = 'La fecha de comprobante inicio no puede ser mayor a la fecha de operación fin';
                }

                if (new Date(this.bsValueFin) < new Date(this.bsValueIni)) {
                    this.isError = true;
                    this.UnselectedItemMessage = 'La fecha de comprobante fin no puede ser menor a la fecha de operación inicio';
                }
                ...
                if (this.isError == true) {
                    ...
                    swal.fire({
                        ...
                        if (result.value) {
                            ...
                            this.AtpReportService.ProcessReportsCuentasxcobrar(data).subscribe(
                                res => {
                                    this.foundResults = res.GenericResponse;

                                    if (this.foundResults != null && this.foundResults.length > 0) {
                                        this.totalItems = res.TotalRowNumber;
                                        this.excelService.generateReportCuentasxcobrarExcel(this.foundResults, 'Reporte CuentasxCobrar -');
                                    }
                                    else {
                                        this.totalItems = 0;
