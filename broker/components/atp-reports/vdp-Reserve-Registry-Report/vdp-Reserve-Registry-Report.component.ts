...
export class VdpReserveRegistryReportComponent implements OnInit {
    ...
    exportToExcel() {
        if (this.foundResults != null && this.foundResults.length > 0)
            this.excelService.generateVdpReserveRegistryReport(this.foundResults, 'Reporte_Polizas_VDP_');
        else
    ...
    ReportRegistryReserve() {
        ...
        if (this.isError == true) {
            ...
            if ((this.bsValueIni != null && this.bsValueFin != null)) {
                ...
                if (!isValid) {
                    this.isError = true;
                    this.UnselectedItemMessage = 'Debe consultar hasta 1 año como máximo.';
                }

                if (new Date(this.bsValueIni) > new Date(this.bsValueFin)) {
                    this.isError = true;
                    this.UnselectedItemMessage = 'La fecha de pago inicio no puede ser mayor a la fecha de pago fin';
                }

                if (new Date(this.bsValueFin) < new Date(this.bsValueIni)) {
                    this.isError = true;
                    this.UnselectedItemMessage = 'La fecha de pago fin no puede ser menor a la fecha de pago inicio';
                }
                            ...
                            this.AtpReportService.ReportRegistryReserve(data).subscribe(
                                res => {
                                    ...
                                    if (this.foundResults != null && this.foundResults.length > 0) {
                                        ...
                                        this.excelService.generateVdpReserveRegistryReport(this.foundResults, 'Reporte_ReservasReaseguros_VDP_');
                                    }
                                    else {
                                        this.totalItems = 0;
