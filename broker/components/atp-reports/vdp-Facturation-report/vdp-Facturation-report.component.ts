...
export class VdpFacturationReportComponent implements OnInit {
    ...
    exportToExcel() {
        if (this.foundResults != null && this.foundResults.length > 0)
            this.excelService.generateReportFacturationExcel(this.foundResults, 'ReporteTecnicoVDP');
        else
                ...
                if (this.isError == true) {
                    ...
                    swal.fire({
                        ...
                        if (result.value) {
                            ...
                            this.AtpReportService.ProcessReportFacturationVDP(data).subscribe(
                                res => {
                                    this.foundResults = res.genericResponse;

                                    if (this.foundResults != null && this.foundResults.length > 0) {
                                        this.totalItems = res.totalRowNumber;
                                        this.excelService.generateReportFacturationExcel(this.foundResults, 'Reporte Facturación VDP -');
                                    }
                                    else {
                                        this.totalItems = 0;
