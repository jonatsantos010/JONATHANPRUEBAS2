...
export class PdSctrRequestStatusComponent implements OnInit {
  ...
  */
  ...
  openDetails(item: any) {
    ...
    this.perfil == 32 ? item.Mode = 'Visualizar' : (item.Mode = item.TramiteProcess == 1 && item.typeTransac != 'Emision' && item.Mode != 'Enviar' ? 'Visualizar' : item.Mode);
        item.From = '';
  ...
  emitRenew(evt, selection: any, idTipo: number){
    ...
    this.policyService.valTransactionPolicy(selection.QuotationNumber, idTipo).subscribe(
        res => {
            if (res.P_COD_ERR == '0') {
